<#
.SYNOPSIS
    Checks that the HST eChart monitor on this server is running, polling, recording, and able to alert.

.DESCRIPTION
    Run from an elevated Windows PowerShell console on a server where Install-HSTMonitor.ps1 was run.
    ...
    sends a real DOWN and RESOLVED email and leaves one short outage in HST-eChart-Outages.csv and the drops log.

.PARAMETER OutageDrill
    Runs the outage drill after the checks.

.PARAMETER InstallDir
    Monitor folder. Default C:\ProgramData\DIT\HSTProbe.
...
param(
    [switch]$OutageDrill,
    [string]$InstallDir = 'C:\ProgramData\DIT\HSTProbe',
    [string]$TaskPath = '\DIT\',
    [string]$TaskName = 'HST eChart Monitor',
    [int]$DrillCleanupMinutes = 10
)

$script:Results = New-Object System.Collections.ArrayList
$script:DrillMarker = '# HST monitor outage drill. Safe to delete.'
$script:DrillCleanupTaskName = 'HST eChart Monitor drill cleanup'
...
function Get-RecentPoll {
    ...
    $files = @(Get-ChildItem -Path $Folder -Filter 'HST-eChart-Latency_*.csv' -ErrorAction SilentlyContinue | Where-Object { $_.Name -match '^HST-eChart-Latency_\d{6}\.csv$' } | Sort-Object Name | Select-Object -Last 2)
    foreach ($f in $files) {
...
function Invoke-OutageDrill {
    ...
    try {
        ...
        while ((Get-Date) -lt $blockedAt.AddSeconds($downBudget)) {
...
Write-Host ""
Write-Host "HST eChart monitor health check on $env:COMPUTERNAME at $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')" -ForegroundColor Cyan
...
$monitorPath = Join-Path $InstallDir 'Watch-HSTeChartUptime.ps1'
if (-not (Test-Path $monitorPath)) {
    Write-Check FAIL "No monitor script at '$monitorPath'. The installer has not been run on this server."
    exit 1
}
...
Write-Check INFO "Site '$($cfg.SiteName)' polls $($cfg.Url) every $($cfg.IntervalSeconds) s with a $($cfg.TimeoutSeconds) s timeout, declares DOWN after $($cfg.DownThreshold) failures in a row, and alerts by $($cfg.MailMethod) to $(@($cfg.MailTo) -join ', ')."
if ($null -eq $cfg.SlowWindowMinutes) { Write-Check WARN "This monitor was installed before slow alerts and daily summaries existed. Re-run Install-HSTMonitor.ps1 to add them." }
else {
...
if (-not (Test-Path $hbPath)) { Write-Check FAIL "No heartbeat file. The monitor has never polled on this server." }
    ...
    if ($hb) {
        if ($hb.IsDown) { Write-Check WARN "The monitor considers HST eChart DOWN right now, since $($hb.OutageStartLocalStr), after $($hb.ConsecutiveFailures) failed polls in a row." }
        elseif ($hb.IsSlow) { Write-Check WARN "The monitor considers HST eChart SLOW right now, since $($hb.SlowStartLocalStr), worst response $($hb.SlowWorstMs) ms." }
...
$lastPollOk = $null
if ($day.Count -eq 0) { Write-Check FAIL "No polls recorded in the last 24 hours in HST-eChart-Latency_*.csv." }
else {
    $last = $day[-1]
...
$dropPath = Join-Path $InstallDir 'HST-eChart-Drops.log'
...
$transcripts = @(Get-ChildItem -Path $InstallDir -Filter 'HST-eChart-Monitor_*.log' -ErrorAction SilentlyContinue | Where-Object { $_.LastWriteTime -ge (Get-Date).AddHours(-24) })
...
switch ("$($cfg.MailMethod)") {
    ...
    'Authenticated' {
        ...
        $tcp = Test-TcpPort -HostName $cfg.SmtpServer -Port ([int]$cfg.SmtpPort)
        if ($tcp) { Write-Check FAIL "Cannot connect to $($cfg.SmtpServer):$($cfg.SmtpPort): $tcp." } else { Write-Check PASS "Connected to $($cfg.SmtpServer):$($cfg.SmtpPort)." }
    }
    default {
        $tcp = Test-TcpPort -HostName $cfg.SmtpServer -Port ([int]$cfg.SmtpPort)
        if ($tcp) { Write-Check FAIL "Cannot connect to $($cfg.SmtpServer):$($cfg.SmtpPort): $tcp." } else { Write-Check PASS "Connected to $($cfg.SmtpServer):$($cfg.SmtpPort)." }
